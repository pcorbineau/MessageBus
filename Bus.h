...
private:
    ...
    std::queue<std::pair<std::unique_ptr<BaseMessage>, std::shared_ptr<BaseHandler>>> _message_queue;
...
Bus::~Bus()
{
    ...
    while (!_message_queue.empty()) {
        auto& [msg, handler] = _message_queue.front();
        handler->publish(std::move(msg));
        _message_queue.pop();
    }
}
